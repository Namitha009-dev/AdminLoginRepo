...
export class SearchAssociateComponent implements OnInit {
  ...
  responseData :  any;
  searchOption:string;
  loggedinHrName:string;
  responseDataLoggedinHr:any;

  constructor(private httpClient:HttpClient,private router:Router) { }

  ngOnInit(): void {
    let responseUrl = this.httpClient.get("http://localhost:9095/hrManager/getHRDetails");
      responseUrl.subscribe((responseDataLoggedinHr) => {
      this.responseDataLoggedinHr = responseDataLoggedinHr;
      console.log(responseDataLoggedinHr); 
      this.loggedinHrName = this.responseDataLoggedinHr.hrName;
    });
